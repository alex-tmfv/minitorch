...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert 0.0 <= s <= 1.0
    # One minus sigmoid equals sigmoid of the negative
...
def test_other(a: float) -> None:
    ...
    relu(x) + relu(-x) == |x|
    """
    assert_close(add(relu(a), relu(neg(a))), abs(a))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    total1 = add(sum(ls1), sum(ls2))
    total2 = sum(addLists(ls1, ls2))
    assert_close(total1, total2)
